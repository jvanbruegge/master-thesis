...
lemma id_on_helper: "\<lbrakk> id_on (A - B) g ; B \<inter> imsupp f = {} ; imsupp x \<inter> imsupp f = {}  \<rbrakk> \<Longrightarrow> id_on (f ` x ` A - x ` B) g"
        

(*

        apply (rule iffD2[OF tmp.inject])
        apply (subst test3_pre.set_map, (rule assms supp_id_bound bij_id supp_inv_bound bij_imp_bij_inv | assumption)+)+

        (*apply (rule exE[OF ex_avoiding_bij[of g _ _ "imsupp f"]])
                apply assumption+
              apply (rule cinfinite_imp_infinite[OF test3_pre.UNIV_cinfinite])
             prefer 6
             apply (erule conjE)+*)
        ...
        apply (subst test3_pre.set_map test3_pre.map_comp, (rule assms supp_id_bound bij_id supp_inv_bound bij_imp_bij_inv | assumption)+)+
        apply (unfold id_o o_id)
        apply (subst inv_o_simp1)
         apply assumption
        apply (subst tmp.vvsubst_rrename[symmetric], (rule supp_inv_bound bij_imp_bij_inv | assumption)+)
        apply (unfold comp_def image_comp)
        apply (subst tmp.set_maps trans[OF comp_apply[symmetric] tmp.vvsubst_comp[symmetric]], (rule assms supp_inv_bound supp_comp_bound cinfinite_imp_infinite[OF test3_pre.UNIV_cinfinite] | assumption)+)+
        apply (unfold id_o o_id)
        apply (rule conjI)
        ...
        apply (rule conjI)
         ...
         defer
         apply (rule test3_pre.map_cong)
                    apply (rule assms bij_id supp_id_bound refl)+
          apply (rule tmp.vvsubst_cong)
             apply (rule assms supp_comp_bound supp_inv_bound cinfinite_imp_infinite[OF test3_pre.UNIV_cinfinite] | assumption)+

        subgoal for z4 a
          apply (rule case_split[of "a \<in> set3_test3_pre x"])
           defer
           apply (drule id_onD)
            apply (rule DiffI)
             apply (rule UN_I)
